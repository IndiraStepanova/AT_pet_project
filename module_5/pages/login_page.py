...
class LoginPage(BasePage):
    def register_new_user(self, email, password):
        email_field = self.browser.find_element(*LoginPageLocators.REGISTER_USERNAME_FIELD)
        email_field.send_keys(email)
        password_field = self.browser.find_element(*LoginPageLocators.REGISTER_PASSWORD_FIELD)
        password_field.send_keys(password)
        re_password_field = self.browser.find_element(*LoginPageLocators.REGISTER_PASSWORD_REPEAT_FIELD)
        re_password_field.send_keys(password)
        submit_button = self.browser.find_element(*LoginPageLocators.REGISTER_PASSWORD_FIELD)
        submit_button.click()
    ...
    def should_be_register_form(self):
        ...
        assert self.is_element_clicable(*LoginPageLocators.REGISTER_SUBMIT_BUTTON), "Submit button is not clicable!"